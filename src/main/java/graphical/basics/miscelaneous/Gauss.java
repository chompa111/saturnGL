...
public class Gauss extends Animation {
    @Override
    ...
    public void buildAnimation() {
        ...
        add(um);
        Animations.fadeInGrow(um, seconds(1)).execute();
        ...
        for(int i=1;i<step1.getGobjects().size();i+=2){
            ...
            int finalI = i;
            var t1= new SupplierTask(()->{
                var n=step1.subGroup(finalI,finalI+1);
                add(n);
                return Animations.fadeInGrow(n,seconds(1));
            });
...
//        LatexGobject.indexsize(step12);
        ...
        add(step12Except);
        Animations.fadeInGrow(step12Except,seconds(1)).execute();
