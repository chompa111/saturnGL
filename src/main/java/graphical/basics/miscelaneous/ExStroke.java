...
public class ExStroke extends Animation {
    @Override
   ...
   //     circle.setStrokeColorHolder(new ColorHolder(Color.red));
        add(circle);
        Animations.strokeAndFill(circle).execute();
       ...
       var sh=(ShapeGobject) new TextGobject("A", Location.at(500,500),Color.white).getGobjects().get(0);
      // sh.setStrokeColorHolder(new ColorHolder(Color.blue));
        ...
        Animations.t3b1b(circle,sh,seconds(1)).execute();
    }
