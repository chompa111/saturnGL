...
public class ExCog extends Animation {
    @Override
    ...
    public void buildAnimation() {
        ...
        Animations.fadeInGrow(l, seconds(2)).execute();
...
//             c.getScale().setValue(0.08+Math.random()*0.1);
            ...
            tasks.add(new SupplierTask(() -> {
                ...
                return wait(1).andThen(Animations.fadeIn(c, seconds(3))
        ...
        for (int i = 0; i < 200; i++) {
            ...
            tasks.add(new SupplierTask(() -> {
                ...
                return wait(finalI + 1).andThen(Animations.fadeIn(c, seconds(3)).parallel(new MeanSpeedTransformation(Arrays.asList(progress), (vel / (radius * radius)), seconds(tempo))));
